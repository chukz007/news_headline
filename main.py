...
from model import LlamaModel, Inferencer
...
if __name__ == "__main__":
    ...
    #parser.add_argument("--model_name", help="Name of the model on hf")
    ...
    model = LlamaModel(model_id=model_id, hf_auth=hf_token, max_length=256)
    ...
    for prompt in prompts[:n]:
        print(prompt)
        result = inferencer.evaluate(prompt)
        pred.append(result)
        print(result)
        print('-'*100)
    ...
    with open(os.path.join(write_path, filename), "w", encoding="utf8") as f:
